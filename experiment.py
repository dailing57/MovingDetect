...
class SceneFlowExp(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('train_loss', loss, sync_dist=True, on_step=False, on_epoch=True, logger=True, prog_bar=False, reduce_fx=torch.mean)
        self.log_dict(metrics, on_step=False, on_epoch=True, logger=True, prog_bar=False, reduce_fx=torch.mean)
    ...
    def _test_val_step(self, batch, batch_idx, split):
        ...
        self.log('val_loss', loss, sync_dist=True, on_step=False, on_epoch=True, logger=True, prog_bar=False, reduce_fx=torch.mean)
        self.log_dict(metrics, on_step=False, on_epoch=True, logger=True, prog_bar=False, reduce_fx=torch.mean)
