...
'''
平滑损失用于使得预测的流场更加平滑,避免出现过于剧烈的变化。
平滑损失分为两个部分: 基于k近邻的损失和基于球形邻域的损失。
k近邻损失是计算当前点和其k个最近邻点之间的差距,这个损失可以避免过于剧烈的变化；
球形邻域损失是基于每个点的球形邻域内所有点的平均流场和该点自身流场的差距，
这个损失可以确保相邻的点有相似的流场。
这两个部分的损失都有一个权重系数，可以通过调整这个系数来平衡它们的影响。    
'''
