...
class FlowRegressor(nn.Module):
    ...
    def forward(self, pc1_l_loc, corr_feats): # 局部坐标，两个点云之间的特征匹配结果
        _, x = self.sa1(pc1_l_loc[2], corr_feats)
...
class GlobalCorrLayer(nn.Module): # 计算全局对应关系
    def __init__(self, npoint, use_instance_norm):
    ...
    def calc_corr_mat(self, pcloud1, pcloud2, feature1, feature2):
        ...
        distance_matrix = torch.sum(pcloud1 ** 2, -1, keepdim=True) # 计算每个点的L2
        distance_matrix = distance_matrix + torch.sum( # [B,N1,N2] 两两点之间的L2和
            pcloud2 ** 2, -1, keepdim=True
        ).transpose(1, 2)
        distance_matrix = distance_matrix - 2 * torch.bmm(pcloud1, pcloud2.transpose(1, 2)) # bmm 得到[B,N1,N2] 最后dis_mat就是得到(p1 - p2)^2
        support = (distance_matrix < self.support_th).float() # 在一定距离之内说明两个点可以匹配
        feature1 = feature1 / torch.sqrt(torch.sum(feature1 ** 2, -1, keepdim=True) + 1e-8) # 归一化
        feature2 = feature2 / torch.sqrt(torch.sum(feature2 ** 2, -1, keepdim=True) + 1e-8)
        C = 1.0 - torch.bmm(feature1, feature2.transpose(1, 2)) # [B, N1, C] [B, C, N2]
        corr_mat = torch.exp(-C / eps) * support # [B, N1, N2]
        return corr_mat
    ...
    def forward(self, pc1_l_glob, pc2_l_glob, feats1_glob, feats2_glob):
        ...
        flow0 = (corr_mat @ pcloud2.permute(0, 2, 1).contiguous()) / (row_sum + 1e-8) - pcloud1.permute(0, 2, 1).contiguous() # 偏移量矩阵，这个点点相减好怪，没有对应关系这么搞有啥意义
