...
class ProcessData(object):
    ...
    def __call__(self, data):
        ...
        minn = min(pc1.shape[0], pc2.shape[0])
        pc1 = pc1[np.random.choice(pc1.shape[0], size=minn, replace=False, p=None)]
        pc2 = pc2[np.random.choice(pc2.shape[0], size=minn, replace=False, p=None)]

        if self.pc2_args is not None:
            pc2 = do_pc2_aug(self.pc2_args, pc2)

        sf = pc2[:, :3] - pc1[:, :3] if pc1.shape[0] == pc2.shape[0] else None
        ...
        sf = sf[sampled_indices1] if sf is not None else None
