...
sf_sum = sf[0].cpu().detach().numpy().reshape(-1, 3)
for cur_sf in sf:
    sf_sum += cur_sf.cpu().detach().numpy().reshape(-1, 3)

pc1 = pc1.cpu().numpy().reshape(-1, 3)
pc2 = pc2.cpu().numpy().reshape(-1, 3)
pcd1 = o3d.geometry.PointCloud()
pcd1.points = o3d.utility.Vector3dVector(pc1[:,:3] + sf_sum)
...
# pcdm = pc1 + sf[-1].cpu().detach().numpy().reshape(-1, 3)
